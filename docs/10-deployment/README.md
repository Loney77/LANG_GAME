...
## Топология развёртывания (продакшн)

```
   Мобильный клиент (Expo/APK)
            │  HTTPS (443)
            ▼
   ┌─────────────────────────────────────────────┐
   │              Хост (VPS), сеть Docker          │
   │                                               │
   │   Caddy  ──reverse_proxy──►  server (8137)    │
   │  (80/443,                     Spring Boot      │
   │  Let's Encrypt)                  │ JDBC        │
   │                                  ▼             │
   │                          db: PostgreSQL 16     │
...
Наружу открыты только порты 80 и 443 (Caddy); сервер и база доступны лишь внутри
сети Docker. TLS-сертификат Caddy получает и продлевает автоматически для домена
`SITE_ADDRESS` (при отсутствии своего домена подойдёт `<IP>.nip.io`). Данные базы
сохраняются в именованном томе и переживают пересоздание контейнеров.
